...
class ConsoleLoggers2 implements Logger {
    log(param: string): void {
        console.log('loger2', + param);
    }
}

// Crie uma interface _Database_.
interface Database {
    logger: Logger;                          // _Database_ deve possuir um atributo do tipo `Logger`.
    save(key: string, value: string): void; // _Database_ deve possuir um método `save`, que recebe dois parâmetros, `key` e `value`, ambos _strings_, e não retorna nada (_void_).
  }
  

// Crie uma classe _ExampleDatabase_ que implementa _Database_.
class ExampleDatabase implements Database {
    constructor(public logger: Logger = new ConsoleLogger()) { }     //receber o _Logger_ como parâmetro do construtor, e possuir como valor padrão um _ConsoleLogger_.
...
const logger2 = new ConsoleLoggers2();
