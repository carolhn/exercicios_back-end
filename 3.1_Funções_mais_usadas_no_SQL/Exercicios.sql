...
SUM(salary) AS SOMA_SALARY,
ROUND(AVG(salary), 2) AS MEDIA_SALARY
FROM hr.employees;

-- 6)Escreva uma query que exiba a quantidade de pessoas que trabalham como pessoas programadoras (it_prog).
SELECT job_id, COUNT(*) AS QUANT_JOB FROM hr.employees
...
SELECT job_id, AVG(salary) as SALARIOS FROM hr.employees
WHERE job_id <> 'it_prog'
GROUP BY job_id
ORDER BY SALARIOS DESC;

-- 10)Escreva um query que exiba média salarial e o número de funcionários de todos os departamentos com mais de dez funcionários. Dica: agrupe pelo department_id.
SELECT department_id, AVG(salary) 'media_salary' , COUNT(*) 'number_of_employees'
FROM hr.employees
...
SELECT first_name, last_name, DATEDIFF(CURRENT_DATE(), hire_date) 'data_trabalho' FROM hr.employees;
